...
- User only wants prose description (use `/create-description`)
- User only wants a diagram (use `/create-diagram`)
- User wants a design doc from conversation history (use `/design`)
...
if [ $DESC_EXIT -eq 0 ]; then
  ...
  DESCRIPTION_RESULT="> **Note:** Prose description generation failed. Re-run with \`/create-description\` to generate it separately."
fi

if [ $DIAG_EXIT -eq 0 ]; then
  DIAGRAM_RESULT=$(cat /tmp/document_diagram_result.txt)
else
  DIAGRAM_RESULT="> **Note:** Diagram generation failed. Re-run with \`/create-diagram\` to generate it separately."
fi
```
