...
- After running `/rlm-design`, want to verify implementation follows it

## When NOT to use

- User wants a general code review (use `/rlm-review`)
- User wants to investigate a failure (use `/rlm-diagnose`)
- User wants a design doc created (use `/rlm-design`)
- User wants to research how something works (use `/rlm-research`)
- No design docs exist — suggest running `/rlm-design` first, then validate
...
if [ "$DESIGN_DOC_COUNT" -eq 0 ]; then
  ...
  echo "Run /rlm-design first to generate a design doc, then run /validate." >&2
  exit 1
fi
```
...
message directing the user to run `/rlm-design` first.
