...
- User wants to plan a single feature (use `/rlm-plan-feature`)
- User wants sprint-level planning (use `/rlm-plan-sprint`)
- User wants a design doc (use `/rlm-design`)
- User wants to explore a topic (use `/rlm-research`)
- The project is small enough for a single sprint — use `/rlm-plan-sprint` instead
...
if [ "$ARTIFACT_COUNT" -eq 0 ]; then
  ...
  echo "Consider running /rlm-design → /rlm-plan-feature first to build up artifacts."
fi
```
...
if [ "$ARTIFACT_COUNT" -gt 0 ]; then
  ...
  echo "Consider running /rlm-design → /rlm-plan-feature to build artifact base." >> "$CONTEXT"
  echo "" >> "$CONTEXT"
fi
...
1. **Parallel input gathering:**
   - **Broad artifact discovery** — ALL of `derived/` and `docs/` are
     ...
     running `/rlm-design` followed by `/rlm-plan-feature` to build up an
     artifact base.
