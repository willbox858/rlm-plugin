...
- User only wants a diagnosis report (use `/rlm-diagnose`)
- User wants to implement a feature (use `/rlm-implement`)
- User wants a code review (use `/rlm-review`)
- The fix is trivially obvious — just fix it directly
- User wants to understand root cause without changing code (use `/rlm-diagnose`)
...
# Priority 3: project.json in project root
  ...
  echo ""
  echo "The /rlm-bugfix skill requires a project config with at least a test_command."
...
output, bridging the gap between `/rlm-diagnose` (analysis only) and
`/rlm-implement` (requires a pre-existing plan).
