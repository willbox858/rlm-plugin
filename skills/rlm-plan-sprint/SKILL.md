---
name: rlm-plan-sprint
description: "Create a capacity-aware sprint plan by consolidating stories from multiple feature plans into a prioritized backlog. Trigger when: 'plan the sprint', 'sprint planning', 'what should we work on next', 'prioritize stories', 'sprint backlog', or user wants to organize multiple /rlm-plan-feature outputs into an ordered sprint."
---
...
- Multiple `/rlm-plan-feature` outputs exist and need to be consolidated

## When NOT to use

- User wants to break a single feature into stories (use `/rlm-plan-feature`)
- User wants epic/roadmap-level planning (use `/rlm-plan-epic`)
- User wants a design doc (use `/rlm-design`)
- User wants to explore a topic (use `/rlm-research`)
- No feature plans exist yet — suggest running `/rlm-plan-feature` first
...
if [ "$PLAN_COUNT" -eq 0 ] && [ "$DESIGN_COUNT" -eq 0 ]; then
  ...
  echo "Consider running /rlm-plan-feature first for individual features."
fi
```
...
This skill discovers outputs from `/rlm-plan-feature` and `/rlm-design`
...
suggesting the user run `/rlm-plan-feature` first for individual features.
