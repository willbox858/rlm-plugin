---
name: rlm-update
description: "Regenerate a stale derived document by re-running the original skill's pipeline with fresh code context, presenting old-vs-new diff. Trigger when: 'update', 'refresh this doc', 'regenerate', 'this doc is stale', 'sync the docs', or any derived document (from /rlm-design, /rlm-document, /rlm-research, etc.) needs updating."
---
...
- User wants to create a new doc (use /rlm-design, /rlm-document, /rlm-describe, etc.)
- User wants to validate code against design (use /rlm-validate)
- User wants to implement code (use /rlm-implement)
...
|------|---------|---------------|-------------------|
| `description` | — | /rlm-describe | GC + description synthesis |
| `diagram` | * | /rlm-diagram | GC + diagram synthesis |
| `design` | * | /rlm-design | distill + GC + design synthesis |
| `plan` | `feature` | /rlm-plan-feature | distill + artifact scan + GC + planning |
| `plan` | `sprint` | /rlm-plan-sprint | artifact scan + GC + sprint planning |
| `plan` | `epic` | /rlm-plan-epic | artifact scan + GC + epic planning |
| `validation` | — | /rlm-validate | design doc scan + GC + validation |
| `review` | * | /rlm-review | target GC + pattern GC + review |
| `diagnosis` | — | /rlm-diagnose | symptom-guided GC + design scan + diagnosis |
| `research` | — | /rlm-research | broad GC + gap analysis + synthesis |
| `document` | — | /rlm-document | GC + description + diagram synthesis |
| `distillation` | — | /rlm-distill | session log extraction + tagged synthesis |
...
Run distill + GC in parallel (same as /rlm-design Step 1):
...
Scan for design docs + run GC (same as /rlm-validate Step 1).
