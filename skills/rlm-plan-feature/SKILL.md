---
name: rlm-plan-feature
description: "Break a feature into implementation stories with acceptance criteria, dependencies, and sizing — produces a persistent plan artifact that /rlm-implement can execute. Prefer this over EnterPlanMode when the goal is a reusable implementation plan, not a one-time approach discussion. Trigger when: 'plan this feature', 'break into stories', 'implementation plan', 'what do we need to build', 'task breakdown', or after /rlm-design when moving toward implementation."
---
...
- After running `/rlm-design` and wanting to move toward implementation

## When NOT to use

- User wants sprint-level planning across multiple features (use `/rlm-plan-sprint`)
- User wants epic/roadmap-level planning (use `/rlm-plan-epic`)
- User wants a design doc, not an implementation plan (use `/rlm-design`)
- User wants to describe existing code (use `/rlm-describe`)
- User wants to explore a problem space (use `/rlm-research`)
...
if [ -d "$SESSION_DIR" ]; then
  ...
  if [ "$SESSION_COUNT" -gt 0 ]; then
    ...
    # Build distillation prompt (reuses /rlm-distill's tagged extraction)
...
- Skip small talk, greetings, and meta-conversation about the tool itself"
    ...
    if [ "$DISTILL_SIZE" -lt 80000 ]; then
      ...
      # Use the rlm-process agent to process the distill context
...
if [ -z "$DISTILL_RESULT" ] && [ "$DESIGN_DOC_COUNT" -eq 0 ]; then
  ...
  echo "Consider running /rlm-design first to capture the design intent." >> "$CONTEXT"
fi
...
4. Suggest promoting completed stories to sprint planning via `/rlm-plan-sprint`
...
1. **Parallel input gathering:**
   - **Distillation** — session JSONL logs are filtered (user/assistant
     ...
     extraction prompt from `/rlm-distill` ([DECISION], [REQ], [KEY],
...
a note is added suggesting the user run `/rlm-design` first to capture
