...
- User has a feature plan (from /rlm-plan-feature or manually written) and wants it implemented
...
- After running /rlm-plan-feature and wanting to move to implementation

## When NOT to use

- User wants a design doc (use /rlm-design)
- User wants an implementation plan (use /rlm-plan-feature)
- User wants to review code (use /rlm-review)
- User wants to fix a specific bug (just fix it directly)
- No feature plan exists — suggest /rlm-plan-feature first
...
if [ "$CANDIDATE_COUNT" -eq 1 ]; then
  ...
  echo ""
  echo "Run /rlm-plan-feature first to create an implementation plan."
...
# Priority 3: project.json in project root
  ...
  echo ""
  echo "The /rlm-implement skill requires a project config with at least a test_command."
...
Invoke the implementation-orchestrator agent. It has the rlm-implement-worker
